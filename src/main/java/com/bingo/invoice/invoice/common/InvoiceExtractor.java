...
public class InvoiceExtractor {
	...
	public InvoiceData extract(String filename) throws Exception,ServiceException, IOException {
		...
		//获取第二页
		PDPage secondPage=null;
		int pageWidth1 =0;
		Rectangle gmxx1 = null;
		Rectangle spmc1=null;
		if(pages==2){
			secondPage = (PDPage) allPages.get(1);
			pageWidth1=	Math.round(secondPage.getCropBox().getWidth());
			//第二页税率坐标
			float[] sl_2 = list1.get(1);
			//规格型号坐标
			String ggxh = "规格型号";
			PdfBoxKeyWordPosition kwp1_2 = new PdfBoxKeyWordPosition(ggxh, pdfPath);
			List<float[]> list1_2 = kwp1_2.getCoordinate();
			float[] ggxh_2 = list1_2.get(1);
			//备注坐标
			String bz = "备注";
			PdfBoxKeyWordPosition kwp2_2 = new PdfBoxKeyWordPosition(bz, pdfPath);
			List<float[]> list2_2 = kwp2_2.getCoordinate();
			float[] bz_2 = list2_2.get(1);
			//订单号坐标
			String ddh = "订单号";
			PdfBoxKeyWordPosition kwp3_2 = new PdfBoxKeyWordPosition(ddh, pdfPath);
			List<float[]> list3_2 = kwp3_2.getCoordinate();
			float[] ddh_2 = list3_2.get(1);

			int h2 = Math.round(bz_2[0]) - Math.round(sl_2[0]) -20;
			gmxx1=new Rectangle(Math.round(ggxh_2[1])-15,Math.round(sl_2[0])+10,pageWidth1,h2);

			spmc1=new Rectangle(Math.round(ddh_2[1]-17),Math.round(sl_2[0])+10,Math.round(ggxh_2[1])-15-Math.round(ddh_2[1]),h2);
		...
		//第二页
		PDFTextStripperByArea str1 = new PDFTextStripperByArea();
		str1.setSortByPosition(true);

		if(gmxx1!=null&&spmc1!=null){
			str1.addRegion("GMXX1",gmxx1);
			str1.addRegion("SPMC1",spmc1);
		}
		// 购买方：
		...
		if(secondPage!=null){
			str1.extractRegions(secondPage);
		}
		...
		String gmxxString = "";
		if(pages==1){
			gmxxString = stripper.getTextForRegion("GMXX").replaceAll("　", " ");
		}else{
			gmxxString = str1.getTextForRegion("GMXX1").replaceAll("　", " ");
		}
		...
		String spmcString="";
		if(pages==1){
			spmcString = stripper.getTextForRegion("SPMC").replaceAll(" ", "").replaceAll("　", "");
		}else{
			spmcString = str1.getTextForRegion("SPMC1").replaceAll(" ", "").replaceAll("　", "");
		}
		...
		for(int i=0;i<nameArr.length/2;i++){
			String ss="*"+nameArr[2*(i+1)-1]+"*"+replaceBlank(nameArr[2*(i+1)]);
			nameArrList.add(ss);
		}
		...
		for(int i=0;i<mxArr.length/2;i++){
			String ss="*"+mxArr[2*(i+1)-1]+"*"+replaceBlank(mxArr[2*(i+1)]);
			mxArrList.add(ss);
		}
		...
		for (int i = 0; i < mxArrList.size(); i++) {
			nameList.add(nameArrList.get(i));
		}
		//nameList.add(spName);
		...
		if(pages==1){
			...
			for (int i = 0; i < nameArrList.size(); i++) {
				if (i >= itemList.size())
					continue;
				itemList.get(i).put("hwmc", nameArrList.get(i));
			}
		}
		...
		if(invoice.getType().equals("01"))
		{
			...
			for (Map<String, Object> item : itemList) 
			{
				...
				String [] fwmc=((String) item.get("hwmc")).split("\\*");
				detail1.setFwmc1(fwmc[1]);
				detail1.setFwmc2(fwmc[2]);
		...
		}else
		{
			...
			for (Map<String, Object> item : itemList) 
			{
				...
				String [] fwmc=((String) item.get("hwmc")).split("\\*");
				detail.setFwmc1(fwmc[1]);
				detail.setFwmc2(fwmc[2]);
