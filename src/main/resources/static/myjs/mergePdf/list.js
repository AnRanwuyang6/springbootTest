...
$(function(){
    $('.merge_00').find('ul').attr("class","nav nav-second-level collapse in");
    $('.merge_00').find('ul').attr("expanded","true");
    $('.merge_00').addClass('active')
    $('.merge_01').attr("class", "active");

    //图片上传插件
    //图片上传插件
    $("#input-id").fileinput({
        language: 'zh',                                 //中文
        uploadUrl:'/merge/files',
        ...
        if(response.code=='0000'){
            var imageUrlsId=response.data
            window.location.href="/merge/print?imageUrlsId="+imageUrlsId
        }
    });
})
